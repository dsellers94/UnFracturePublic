...
def GenerateAll():
    ...
    GatherMetaData(Nx, DeltaX, Ny, DeltaY)

    global KReduxInverse
    global KReduxInversePrime
    KReduxInverse = GenerateKReduxInverse()
    KReduxInversePrime = uc.ReduceMatrixPrime(KReduxInverse, Nodes, EdgeNodeIndices, FixedNodeIndices, EdgeNodeKMatrixMap)
...
def GenerateKReduxInverse():
    K = uc.GenerateGlobalMatrix(Nodes, 500, 1)
...
def RecalculateEdgeAndFixedIndices(edgeNodeIndices, fixedNodeIndices):
    for i in range(len(edgeNodeIndices)):
        NewEdgeNodeIndices.append(i)
        if i in fixedNodeIndices:
            NewFixedNodeIndices.append(i)
    edgeNodeIndices = NewEdgeNodeIndices
    fixedNodeIndices = NewFixedNodeIndices

def GatherMetaData(nx, deltaX, ny, deltaY):
    MetaData.append(nx)
    MetaData.append(deltaX)
    MetaData.append(ny)
    MetaData.append(deltaY)

def InvokeCSVWriter(MeshNamePrefix):
    uc.WriteToCSV(MeshNamePrefix, Positions, Triangles, KReduxInverse, KReduxInversePrime, FixedNodeIndices, EdgeNodeIndices, EdgeNodePositions, EdgeNodeKMatrixMap, MetaData)
